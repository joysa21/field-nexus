    ...
    <Card className="overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">
                Run — {run.run_at ? new Date(run.run_at).toLocaleString() : "Unknown time"}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {run.total_issues || 0} issues · {run.total_assigned || 0} assigned · {logs.length} agent steps
              </p>
            </div>
          </div>
      ...
      {expanded && (
        <CardContent className="p-0">
          ...
          >
            {logs.length === 0 ? (
              <span className="text-green-400/40">No agent logs recorded.</span>
            ) : (
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">{runs.length} pipeline runs recorded</p>
      </div>

      {loading ? (
        <div className="text-center text-muted-foreground py-16">Loading logs…</div>
      ) : runs.length === 0 ? (
        <div className="text-center py-20 text-muted-foreground text-sm">
          No agent runs yet. Go to <strong>Run Agents</strong> to start the pipeline.
        </div>
      ) : (
